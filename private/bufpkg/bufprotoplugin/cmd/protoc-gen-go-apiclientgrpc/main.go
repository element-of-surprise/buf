...
func handleGlobal(helper protogenutil.NamedHelper, plugin *protogen.Plugin, goPackageFileSets []*protogenutil.GoPackageFileSet) error {
	...
	for _, goPackageFileSet := range goPackageFileSetsWithServices {
		apiclientgrpcGoImportPath, err := helper.NewPackageGoImportPath(goPackageFileSet, "apiclientgrpc")
		if err != nil {
			return err
		}
	...
	for _, goPackageFileSet := range goPackageFileSetsWithServices {
		goImportPath, err := helper.NewPackageGoImportPath(goPackageFileSet, "apiclientgrpc")
		if err != nil {
			return err
		}
